...
(function($){
	$.fn.bookingForm=function(options){
		return this.each(function(){
			...
			object = {
				url: 'bat/BookTaxiHandler.php', // php-script url
				sender: '', // sender for header in e-mail
				ownerEmail:'dhanvitourandtravels@gmail.com', // destination e-mail, message will be send on this e-mail
				...
				submitData: function(){
				debugger;
				var nameOfCustomer = $( ".name").val();
				var emailOfCustomer = $( ".email").val();
				var cityOfCustomer = $( ".city").val();
				var dateOfJourny = $( ".date").val();
				var contactOfCustomer = $( ".contact").val();
				...
				var subject = "New taxi booking request"
				var overallMessage = "Hi there, There is a new booking request. \r\n Name of customer : "+nameOfCustomer+"\r\n Selected car : "+selectedCar+"\r\n Email : "+emailOfCustomer+"\r\n City : "+cityOfCustomer+"\r\n Contact No. : "+contactOfCustomer+"\r\n Date of journey : "+dateOfJourny+"\r\n Number of adults : "+numberOfAdults+"\r\n Number of children : "+numberOfChildren+"\r\n Message : "+message;
				var ip;
				var googleSecret = "6LcPXCETAAAAAFeJqAAyaQd0i6ZFFtpOJK5lt3nY";
				...
				$.ajax({
						...
						data: { secret : googleSecret, response : captcha, remoteip: ip},
						crossDomain: true,
					...
					$.ajax({
						type: "POST",
						url: object.url,
						data: { subject : subject, overallMessage : overallMessage },
						success: function(results){
							$this
								.find(object.successMessageClass).remove().end()
								.find('[data-type="submit"]').after('<p class="'+className(object.successMessageClass)+'">'+ object.successMessage +'</p>').end()
